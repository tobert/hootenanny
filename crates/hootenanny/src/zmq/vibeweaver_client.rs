...
//!
//! Connects to vibeweaver's ZMQ ROUTER for Python kernel tools.
//! Used by hootenanny to proxy weave_* requests.
...
/// Connection state to vibeweaver
#[repr(u8)]
...
/// Health tracking for vibeweaver connection
...
/// Client for connecting to vibeweaver's ZMQ ROUTER
pub struct VibeweaverClient {
    endpoint: String,
...
impl VibeweaverClient {
    /// Connect to vibeweaver at the given endpoint
    pub async fn connect(endpoint: &str, timeout_ms: u64) -> Result<Self> {
        debug!("Creating DEALER socket for vibeweaver");
        let mut socket = DealerSocket::new();

        // Connect with timeout
        tokio::time::timeout(Duration::from_secs(5), socket.connect(endpoint))
            .await
            .with_context(|| format!("Timeout connecting to vibeweaver at {}", endpoint))?
            .with_context(|| format!("Failed to connect to vibeweaver at {}", endpoint))?;

        info!("Connected to vibeweaver at {}", endpoint);
    ...
    /// Send a payload to vibeweaver and wait for response
    pub async fn request(&self, payload: Payload, traceparent: Option<String>) -> Result<Payload> {
        ...
        debug!(
            "Sending capnp request to vibeweaver ({} bytes)",
            frame.body.len()
        );
    ...
    pub async fn send_heartbeat(&self) -> Result<()> {
        let frame = HootFrame::heartbeat("vibeweaver");
        let frames = frame.to_frames();
...
/// Spawn a heartbeat task for vibeweaver connection monitoring
pub fn spawn_heartbeat_task(
    client: Arc<VibeweaverClient>,
    interval: Duration,
    max_failures: u32,
    ...
    tokio::spawn(async move {
        ...
        info!("💓 Heartbeat monitoring started for vibeweaver");
            ...
            tokio::select! {
                _ = ticker.tick() => {
                    match client.send_heartbeat().await {
                        Ok(()) => {
                            debug!("vibeweaver: heartbeat OK");
                        }
                        Err(e) => {
                            let failures = client.health.record_failure();
                            warn!("vibeweaver: heartbeat failed: {} ({}/{})", e, failures, max_failures);

                            if failures >= max_failures {
                                let state = client.health.get_state();
                                if state != ConnectionState::Dead {
                                    client.health.set_state(ConnectionState::Dead);
                                    warn!("vibeweaver: marked as DEAD after {} consecutive failures", failures);
                                }
                            }
                        }
                    }
                ...
                _ = shutdown.recv() => {
                    info!("vibeweaver: heartbeat task shutting down");
                    break;
                }
            }
        }
