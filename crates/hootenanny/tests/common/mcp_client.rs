...
impl McpClient {
    ...
    /// Connect using an existing session ID (Simulating resume/reconnect)
    pub async fn connect_with_session(base_url: &str, session_id: &str) -> Result<Self> {
        let client = reqwest::Client::new();
        let mut mcp_client = Self {
            base_url: base_url.to_string(),
            client,
            session_id: None, // Will be set by establish_session
            responses: Arc::new(Mutex::new(HashMap::new())),
    ...
    async fn establish_session(&mut self) -> Result<()> {
        self.establish_session_with_id("").await
    }

    async fn establish_session_with_id(&mut self, session_id_hint: &str) -> Result<()> {
        // Pass sessionId query param if we are trying to resume
        let sse_url = if !session_id_hint.is_empty() {
            format!("{}/sse?sessionId={}", self.base_url, session_id_hint)
        } else {
        ...
        let received_session_id = timeout(Duration::from_secs(5), session_rx)
            .await
            .context("Timeout waiting for session ID from SSE stream")?
            .context("Failed to receive session ID from listener")?;

        eprintln!("[MCP] Got session ID: {}", received_session_id);
...
fn extract_session_id(text: &str) -> Option<String> {
    eprintln!("[EXTRACT] Attempting to extract from: {:?}", text);
    for line in text.lines() {
        ...
        // Handle JSON format: data: {"uri":"/mcp/message?sessionId=..."}
        if line.starts_with("data: {") {
            eprintln!("[EXTRACT] Found JSON data line");
            let json_str = line.strip_prefix("data: ")?;
            eprintln!("[EXTRACT] JSON string: {:?}", json_str);
            if let Ok(value) = serde_json::from_str::<Value>(json_str) {
                eprintln!("[EXTRACT] Parsed JSON: {:?}", value);
                if let Some(uri) = value.get("uri").and_then(|v| v.as_str()) {
                    eprintln!("[EXTRACT] Found URI: {}", uri);
                    if let Some(session_id) = uri.split("sessionId=").nth(1) {
                        eprintln!("[EXTRACT] Found JSON format session ID: {}", session_id);
    ...
    eprintln!("[EXTRACT] No session ID found");
    None
}
...
mod tests {
    ...
    #[test]
    fn test_extract_session_id_raw_format() {
