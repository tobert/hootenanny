...
This directory contains the shared memory files that all agents (Claude, Gemini, GPT, etc.) use to maintain context across sessions and enable seamless collaboration on the HalfRemembered Launcher project.
...
### With Models
- All models reference via CLAUDE.md
...
### HalfRemembered Launcher Context
This memory system is used for developing the SSH-based RPC launcher that enables server-initiated push operations to clients behind NAT/firewalls.

Key areas tracked:
- **SSH Protocol**: Persistent connections, multiplexed channels
- **File Sync**: Transfer reliability, checksums, atomic writes
- **Auto-Execute**: File watching, execution workflows
- **Reconnection**: Backoff strategies, connection management
- **Rust/Tokio**: Async patterns, error handling

### Technology Focus
- Rust with tokio async runtime
- russh for SSH protocol
- bincode for efficient serialization
- anyhow for error handling

### Common Patterns
See PATTERNS.md for comprehensive list, including:
- SSH-only security model
- Client-initiated connections for NAT traversal
